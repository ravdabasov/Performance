...
import com.example.smarttaskmanager.presentation.viewmodel.TaskChatViewModel
import com.example.smarttaskmanager.util.formatFull

/**
 * Tələb: "tapşırıq barədə daxildə chat bölməsi" - komanda üzvləri hər task-ın
 * öz müzakirə bölməsində real-vaxt yazışa bilir. Sağ üstdə "Söhbəti təmizlə" düyməsi var.
 */
...
fun TaskChatScreen(
    ...
    Scaffold(
        ...
        if (state.messages.isEmpty()) {
            Box(
                ...
                Text(
                    "Hələ mesaj yoxdur. Bu task barədə ilk mesajı yazın.",
            ...
            LazyColumn(
                ...
                items(state.messages, key = { it.id }) { message ->
                    ChatBubble(message = message, isOwn = message.authorUid == viewModel.currentUid)
                }
            }
        }
    }
    ...
    if (showClearConfirm) {
        AlertDialog(
            ...
            text = { Text("Bu task-ın bütün mesajları HAMI üçün silinəcək. Bu əməliyyat geri qaytarıla bilməz. Davam edilsin?") },
            confirmButton = {
                TextButton(onClick = {
                    viewModel.clearChat()
...
@Composable
private fun ChatBubble(message: TaskComment, isOwn: Boolean) {
    Row(
